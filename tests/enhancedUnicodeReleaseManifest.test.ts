...
function spec(): EnhancedUnicodeReleaseSpec {
  return {
    ...
    release: {
      ...
      tag: "enhanced-unicode-v0.3.0",
      notes: "docs/notes.md",
    },
    targets: [
      {
        target: "example-target",
        archive: "typsastra-enhanced-unicode-v0.3.0-example-target.zip",
        executable: "typst",
      },
    ],
  };
...
describe("Enhanced Unicode release manifest", () => {
  test("records immutable asset identity", async () => {
    ...
    expect(manifest.assets[0]).toMatchObject({
      ...
      downloadUrl: `https://github.com/Sovichea/typsastra/releases/download/enhanced-unicode-v0.3.0/${archive}`,
    });
  });
