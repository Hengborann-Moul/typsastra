# Typsastra v0.6.1 release notes

Typsastra v0.6.1 contains the complete v0.6.0 feature set and adds an urgent
PDF loading optimization for substantially lower peak memory use in release
builds.
...
- Replaced whole-file PDF transfer into the WebView with progressive,
  range-based loading.
- Reduced the transient memory spike caused by copying a complete large PDF
  through Tauri IPC before PDF.js could open it.
- Kept range loading responsive after distant scrollbar jumps by progressively
  fetching nearby data and prioritizing the visible pages when scrolling
  settles.
- Staged immutable preview generations so Windows does not need to replace a
  PDF file that the active preview is still reading when switching between
  Normal and Draft modes.
- Closed retired range sources and removed their temporary generation files
  after the replacement preview takes ownership.
- Retained full-buffer loading behind development benchmark commands for
  regression comparison.
- Published the full-buffer-versus-range measurements and the resulting memory
  performance contract in [BENCHMARKS.md](https://github.com/Sovichea/typsastra/blob/v0.6.1/docs/BENCHMARKS.md) and
  [PERFORMANCE_GATES.md](https://github.com/Sovichea/typsastra/blob/v0.6.1/docs/PERFORMANCE_GATES.md).
...
- Published Normal-versus-Draft measurements for optimized and pathological
  image fixtures in [BENCHMARKS.md](https://github.com/Sovichea/typsastra/blob/v0.6.1/docs/BENCHMARKS.md).
...
See the [v0.6.0 feature showcase](https://github.com/Sovichea/typsastra#v060-feature-showcase)
