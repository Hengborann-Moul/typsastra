...
Typsastra is a complex-script-first writing environment for Typst, designed for research papers, technical documentation, theses, books, and other long-form documents.
...
Typsastra must not be reduced to a “Unicode-friendly Typst editor” or presented as “Typst with better Khmer rendering.” Unicode correctness is foundational, but it is not the complete product.
...
> Typsastra is a complex-script-first Typst writing environment, with Khmer as the first deeply supported language.
...
4. **The workflow must scale.** The same design must remain understandable and responsive for a short note, a thesis, a technical proposal, or a long book without making the Typst source dependent on Typsastra.

Typsastra’s long-term goal is to make academic and technical writing more accessible for languages that have historically been underserved by documentation and publishing tools. Product and architecture decisions should be evaluated against that goal.
...
6. **Source remains portable.** Typsastra metadata and caches must not contaminate saved Typst source or become required to compile a project elsewhere.
...
Typsastra already has several parts of this architecture:
...
Typsastra should recognize that files participate in different parts of the writing workflow:
...
generated Typsastra cache
```

File roles control navigation and preview behavior, not whether the files remain ordinary portable project files. Typsastra metadata must never be required by the Typst compiler.
...
The main document depends on includes, imports, templates, bibliographies, figures, and data. Typsastra should maintain a lightweight dependency graph so a change can invalidate the correct document without rescanning or restarting every open preview.
...
Render-on-type may need temporary snapshot files when the compiler cannot consume unsaved memory overlays. Snapshot writes must be atomic, revisioned, isolated under `.typsastra`, and invisible to normal workspace navigation.
...
- [ ] A project remains compilable using the standard Typst toolchain outside Typsastra.
...
- [x] **P1.2 — Rewrite public positioning.** Update the README, repository description, About content, and release-document templates to use the complex-script-first description. README, release workflow, and GitHub repository metadata are aligned. Typsastra does not currently have an About dialog.
...
- [x] The README answers what Typsastra is, who it serves, and why Khmer has special depth without implying Khmer-only scope.
...
- [x] Installing a dictionary refreshes capabilities without restarting Typsastra.
...
- Hide Typsastra caches from the workspace and never require them for external Typst compilation.
...
- [x] **P6.3 — Implement standalone preview rules.** This implementation was subsequently disabled after source-sync reliability regressions. Its portable replacement is deferred to a dedicated future milestone and is not part of v0.8.0.
...
- [x] **P6.9 — Isolate caches.** Hide `.typsastra`, clean obsolete cache entries safely, and verify external Typst compilation never depends on cache contents.
...
- [x] The same source project compiles with the standard Typst toolchain outside Typsastra.
...
`P8.1` remains open for promotion: Lao was selected and its Unicode, ICU4X, dictionary, version, and license sources are confirmed, but Typsastra still needs a fluent Lao maintainer or reviewer. The provider therefore remains experimental enhanced support rather than deep/stable support.
...
This direction is successful when Typsastra can add another deeply supported language through explicit policy and provider modules, without changing Khmer behavior or generic editor integration, while remaining reliable for real multi-file research documents.

Khmer should demonstrate the depth of Typsastra's language support. It should not define the limit of who Typsastra can serve.
