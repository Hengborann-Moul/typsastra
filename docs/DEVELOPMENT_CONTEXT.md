...
- Khmer reference fixtures live under `tests/fixtures/khmer/` and lock editing boundaries, deletion, selection, provider tokens, upstream byte spans, CodeMirror UTF-16 ranges, correction spans, and completion replacements to submodule commit `031fc60bcf29dbdd117d9ab04c5b746032d6ab0a` (v0.2.0-rc.3). Spellcheck uses upstream `SpellingAccuracy::Visual` so COENG DA/TA variants are equivalent without changing curated completion output. Run `bun run test:khmer` plus the native `khmer_reference_provider_fixtures_are_locked` test after any related change.
...
- Docked preview uses Typsastra's virtualized PDF viewer. It supports persistent document-color, hue-preserving dark, and experimental full-inversion modes without changing exported PDFs. Forward and inverse sync use Tinymist source-map positions; PDF text matching and DOM text refinement are intentionally removed because they were unreliable for Khmer, repeated text, and generated preview files. PDF.js decoder assets from the pinned `pdfjs-dist` package are served under `/pdfjs-wasm/` in development and emitted to the same path in release builds so scanner-generated CCITT/JBIG2 foreground layers render consistently. Current behavior is documented in `PREVIEW_INTERCEPTION.md`.
