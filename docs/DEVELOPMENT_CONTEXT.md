...
- Imported sources currently preview through the configured main document. Independent standalone roots remain disabled; the portable Full Document/Active File replacement is planned for v0.8.0 and later hardened under `V1X-P.1`. v0.5.2 supports debounced PDF-on-type for short documents and PDF-on-save for long or resource-intensive documents. A bounded SVG renderer is deferred until after v1.0 and remains only a measurement-driven experiment.
