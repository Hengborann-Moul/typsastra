...
- **Preview (`preview/`):** Preview tasks use deterministic unique IDs per root and the user-selected refresh policy. Imported files currently use the configured main-document preview. Independent standalone roots remain disabled pending the portable v0.6.0 replacement and v1.x hardening under `V1X-P.1`.
