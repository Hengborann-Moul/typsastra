...
7. **Font Management:** `font_store.rs` installs fonts per user and inventories operating-system fonts plus user-configured private local directories. Source-editor selectors must remain limited to operating-system fonts because private compiler fonts are not loaded into the WebView. Document Typography groups Typst built-in, private local, and system families in that order and passes private directories to every Tinymist compile/LSP path. Reject ambiguous private/system family collisions, never copy private font binaries into a project or export, and restart the active compiler session after the global directory list changes. `editor/fontCatalog.ts` owns optional script recommendations, not selector inventory: prefer the matching MiSans family and use script-specific Noto Sans when MiSans has no family. Never download before explicit consent, and remember declined recommendations. Only Fira Mono and MiSans Latin may be bundled.
